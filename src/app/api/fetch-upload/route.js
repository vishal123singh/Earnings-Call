...
const earningsTranscriptsAPIKey =
  process.env.EARNINGS_CALLS_TRANSCRIPTS_API_KEY;
...
function parseTranscript(rawText) {
  ...
  if (currentSpeaker)
    transcriptArray.push({ speaker: currentSpeaker, text: currentText.trim() });
...
const handler = async ({ ticker, year, quarter }) => {
  try {
    const transcript = await fetchEarningsCallsTranscript(
      ticker,
      year,
      quarter,
    );
